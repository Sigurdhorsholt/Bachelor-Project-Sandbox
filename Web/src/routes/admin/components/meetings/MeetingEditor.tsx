...
import { Box, Container, Paper, Typography, TextField, MenuItem, Button, Divider, IconButton, Tooltip } from "@mui/material";
...
export default function MeetingEditor() {
  ...
  const [draft, setDraft] = React.useState<{ title: string; startsAtLocal: string; status: string }>({ title: "", startsAtLocal: "", status: "Draft" });
  React.useEffect(() => {
    if (meeting) {
  ...
  function asErrorText(err: any): string {
    ...
    if (err.data) {
      ...
      if (d.errors) {
        try { return Object.values(d.errors).flat().join("; "); } catch { /* ignore */ }
      }
    }
  ...
  async function addAgenda() {
    ...
    await createAgendaItem({ meetingId: meeting.id, title: `New item ${(meeting.agenda.length) + 1}` }).unwrap().catch(() => {});
    refetch();
  }
  ...
  async function addProposition(itemId: string) {
    ...
    if (!question) return;
    await createProposition({ meetingId: meeting.id, itemId, question, voteType: "YesNoBlank" }).unwrap().catch(() => {});
    refetch();
  }
  ...
  if (!id) {
    return <Container maxWidth="lg" className="py-6"><Typography>Missing meeting id.</Typography></Container>;
  }
  if (isFetching && !meeting) {
    return <Container maxWidth="lg" className="py-6"><Typography>Loading…</Typography></Container>;
  }
  if (isError || !meeting) {
    return <Container maxWidth="lg" className="py-6"><Typography>Meeting not found.</Typography></Container>;
  }
    ...
    <Box className="min-h-screen flex flex-col">
      <Container maxWidth="lg" className="py-4 md:py-6">
        ...
        <Paper className="p-4 md:p-5 rounded-2xl border border-slate-100 mb-4" elevation={0}>
          ...
          {isLocked && <Typography variant="body2" className="!text-red-600 mt-2">This meeting is finished and cannot be edited.</Typography>}
        </Paper>
        ...
        <Paper className="p-4 md:p-5 rounded-2xl border border-slate-100" elevation={0}>
          <div className="flex items-center justify-between">
            ...
            <Tooltip title={isLocked ? "Meeting is finished" : "Add agenda item"}>
              <span>
                <Button startIcon={<AddIcon />} variant="outlined" size="small" className="!rounded-lg" onClick={addAgenda} disabled={isLocked}>Add item</Button>
              </span>
            </Tooltip>
          </div>
              ...
              <Paper key={a.id} className="p-3 rounded-xl border border-slate-200" elevation={0}>
                <div className="flex items-center gap-2">
                  ...
                  <div className="ml-auto flex items-center gap-1">
                    <Tooltip title="Edit title"><span><IconButton size="small" onClick={() => renameAgenda(a.id)} disabled={isLocked}><EditIcon fontSize="small" /></IconButton></span></Tooltip>
                    <Tooltip title="Remove"><span><IconButton size="small" onClick={() => removeAgenda(a.id)} disabled={isLocked}><DeleteIcon fontSize="small" /></IconButton></span></Tooltip>
                  </div>
                </div>
                <div className="mt-2 text-sm text-slate-600">Propositions: {a.propositions.length}</div>
                <div className="mt-2">
                  <Button size="small" variant="text" startIcon={<AddIcon />} onClick={() => addProposition(a.id)} disabled={isLocked}>Add proposition</Button>
                </div>
              </Paper>
            ))}
            {meeting.agenda.length === 0 && <Typography variant="body2" className="!text-slate-500">No agenda items yet.</Typography>}
          </div>
        </Paper>
      </Container>
    </Box>
